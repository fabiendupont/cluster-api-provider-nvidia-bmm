...
var _ = Describe("NVIDIA BMM Cluster Lifecycle E2E", func() {
	var (
		...
		It("should create a complete HA cluster with 3 control plane and 3 worker nodes", func() {
			...
			secret := &corev1.Secret{
				...
				Data: map[string][]byte{
					"endpoint": []byte(os.Getenv("NVIDIA_BMM_API_ENDPOINT")),
					"orgName":  []byte(os.Getenv("NVIDIA_BMM_ORG_NAME")),
					"token":    []byte(os.Getenv("NVIDIA_BMM_API_TOKEN")),
				},
			}
			...
			By("Waiting for NvidiaBMMCluster to be ready")
			Eventually(func() bool {
				err := k8sClient.Get(ctx, client.ObjectKeyFromObject(nvidiaBmmCluster), nvidiaBmmCluster)
				if err != nil {
					return false
				}
				return nvidiaBmmCluster.Status.Ready
			}, clusterCreationTimeout, pollInterval).Should(BeTrue())

			By("Verifying VPC was created")
			Expect(nvidiaBmmCluster.Status.VPCID).NotTo(BeEmpty())

			By("Verifying subnets were created")
			Expect(nvidiaBmmCluster.Status.NetworkStatus.SubnetIDs).To(HaveLen(2))
			Expect(nvidiaBmmCluster.Status.NetworkStatus.SubnetIDs).To(HaveKey("control-plane"))
			Expect(nvidiaBmmCluster.Status.NetworkStatus.SubnetIDs).To(HaveKey("worker"))
			...
			By("Waiting for NvidiaBMMCluster to be deleted")
			Eventually(func() bool {
				err := k8sClient.Get(ctx, client.ObjectKeyFromObject(nvidiaBmmCluster), nvidiaBmmCluster)
				return err != nil
