...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "7eb3518c.cluster.x-k8s.io",
	...
	if err := (&controller.NvidiaBMMClusterReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(ctx, mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "NvidiaBMMCluster")
		os.Exit(1)
	}
	if err := (&controller.NvidiaBMMMachineReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
