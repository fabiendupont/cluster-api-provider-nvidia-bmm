...
Phase 2 of the repository separation plan has been completed. The cluster-api-provider-nvidia-bmm now uses the auto-generated Go client from `github.com/NVIDIA/carbide-rest/client` instead of the custom `pkg/cloud` package.
...
**File**: `internal/controller/nvidiabmmcluster_controller.go`
...
**File**: `internal/controller/nvidiabmmmachine_controller.go`
...
#### NvidiaBMMCluster Status

**File**: `api/v1beta1/nvidiabmmcluster_types.go`
...
- Controller handles NVIDIA BMM-specific IP block requirements
...
func (r *NvidiaBMMClusterReconciler) ensureIPBlock(ctx context.Context, clusterScope *scope.ClusterScope, siteID string) (uuid.UUID, error) {
    ...
    ipBlockReq := restclient.CreateIpblockJSONRequestBody{
        Name:            fmt.Sprintf("%s-ipblock", clusterScope.NvidiaBMMCluster.Name),
        Prefix:          "10.0.0.0",
        PrefixLength:    16,
...
**Test files affected:**
- `internal/controller/nvidiabmmcluster_controller_test.go`
- `internal/controller/nvidiabmmmachine_controller_test.go`
...
None - CRD API remains unchanged. Existing NvidiaBMMCluster and NvidiaBMMMachine resources continue to work.
...
**Before:**
```go
import "github.com/NVIDIA/cluster-api-provider-nvidia-bmm/pkg/cloud"
