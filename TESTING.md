...
**Disabled Test Files:**
- `internal/controller/nvidiabmmcluster_controller_test.go.disabled`
- `internal/controller/nvidiabmmmachine_controller_test.go.disabled`
...
Use `httptest` to create a mock NVIDIA BMM REST API server:
...
```go
func TestNvidiaBMMCluster_Integration(t *testing.T) {
...
import (
    ...
    restclient "github.com/NVIDIA/carbide-rest/client"
    infrastructurev1 "github.com/NVIDIA/cluster-api-provider-nvidia-bmm/api/v1beta1"
    "github.com/NVIDIA/cluster-api-provider-nvidia-bmm/internal/controller"
    "github.com/NVIDIA/cluster-api-provider-nvidia-bmm/pkg/scope"
)

func setupMockNvidiaBMMAPI(t *testing.T) (*httptest.Server, *restclient.ClientWithResponses) {
    mux := http.NewServeMux()
...
func TestNvidiaBMMClusterController_ReconcileVPC(t *testing.T) {
    server, nvidiaBMMClient := setupMockNvidiaBMMAPI(t)
    defer server.Close()

    // Create test NvidiaBMMCluster
    ...
    // Run reconciliation
    reconciler := &controller.NvidiaBMMClusterReconciler{
        // ... setup
    }
